...
class FlexibleThresholdedLoss(nn.Module):
    def __init__(self, use_mse_threshold=True, use_mae_threshold=True, mse_threshold=None, mae_threshold=None,
                 ...
                 mse_clip_ratio=5e1, mae_clip_ratio=5e1):
        """
    ...
    def forward(self, input_img, target_img):
        ...
        if self.use_mse_threshold:
            ...
            mse_thresholded_diff = torch.where(pixel_diff < self.mse_threshold,
                                               torch.tensor(0.0, device=pixel_diff.device), pixel_diff)
            ...
            # Apply clipping: if mse_clip_ratio is provided, skip the thresholded loss for values beyond the clip threshold
            if self.mse_clip_ratio is not None:
                max_mse_loss = self.mse_clip_ratio * self.mse_threshold
                mse_thresholded_l1_loss = torch.where(mse_thresholded_l1_loss > max_mse_loss,
                                                      torch.tensor(0.0, device=mse_thresholded_l1_loss.device),
                                                      mse_thresholded_l1_loss)
                mse_thresholded_l2_loss = torch.where(mse_thresholded_l2_loss > max_mse_loss,
                                                      torch.tensor(0.0, device=mse_thresholded_l2_loss.device),
                                                      mse_thresholded_l2_loss)
        else:
        ...
        if self.use_mae_threshold:
            ...
            mae_thresholded_diff = torch.where(pixel_diff < self.mae_threshold,
                                               torch.tensor(0.0, device=pixel_diff.device), pixel_diff)
            ...
            # Apply clipping: if mae_clip_ratio is provided, skip the thresholded loss for values beyond the clip threshold
            if self.mae_clip_ratio is not None:
                max_mae_loss = self.mae_clip_ratio * self.mae_threshold
                mae_thresholded_l1_loss = torch.where(mae_thresholded_l1_loss > max_mae_loss,
                                                      torch.tensor(0.0, device=mae_thresholded_l1_loss.device),
                                                      mae_thresholded_l1_loss)
                mae_thresholded_l2_loss = torch.where(mae_thresholded_l2_loss > max_mae_loss,
                                                      torch.tensor(0.0, device=mae_thresholded_l2_loss.device),
                                                      mae_thresholded_l2_loss)
        else:
        ...
        total_loss = self.threshold_weight * (
                    mse_thresholded_loss + mae_thresholded_loss) + self.non_threshold_weight * non_thresholded_loss
